			...
			.ready(
					function() {
						...
						$("input[name='chkAll']").click(
								function() {
									$("tbody input[type=checkbox]").prop(
								...
								.click(
										function() {
											if ($('tbody input:checkbox:checked').length == 0) {
												...
												$("#frmBalanceCalc")
														.attr("action",
																"<c:url value='/admin/balCalc/balMultiDecision.do' />");
			...
			<table class="table table-bordered" summary="해당 월의 업체별정산에 관한 표">
				<thead>
					<tr>
						...
						<th class="text-center">정산처리 <input type="checkbox"
							name="chkAll">
						</th>
				...
				<tbody>
					...
					<c:if test="${!empty blist}">
						...
						<c:forEach var="map" items="${blist }">
							<tr>
								...
								<td class="text-right"><fmt:formatNumber pattern="#,###"
										value="${map['BAL_SALES'] }" /> 원</td>
								<td class="text-right">${map['COM_RATE'] }%</td>
								<td class="text-right"><fmt:formatNumber pattern="#,###"
										value="${map['BAL_COMMISSION'] }" /> 원</td>
								<td class="text-center"><c:if
										test="${empty map['BAL_DECISION_DATE']}">
										정산처리 전
										<c:set value="${map['BAL_SALES']+NoSumSales }"
											var="NoSumSales" />
										<c:set value="${map['BAL_COMMISSION']+NoSumCommission }"
											var="NoSumCommission" />
									</c:if> <c:if test="${!empty map['BAL_DECISION_DATE']}">
										<fmt:formatDate value="${map['BAL_DECISION_DATE'] }"
											pattern="yyyy-MM-dd HH:mm" />에 정산처리
										<c:set value="${map['BAL_SALES']+YesSumSales }"
											var="YesSumSales" />
										<c:set value="${map['BAL_COMMISSION']+YesSumCommission }"
											var="YesSumCommission" />
									</c:if></td>
								<td class="text-center"><c:if
										test="${empty map['BAL_DECISION_DATE']}">
										<input type="checkbox" id="chk_${i }"
											name="balanceItems[${i}].balNum" value="${map['BAL_NUM']}">
									</c:if></td>
								<td class="text-center"><c:if
										test="${!empty map['BAL_DECISION_DATE']}">
										<a
											href='<c:url value="/admin/balCalc/balCancel.do?balNum=${map['BAL_NUM']}"/>'>
											<input type="button" value="정산취소">
										</a>
									</c:if></td>
							</tr>
						...
						<tr>
							<td class="text-right" colspan="4"><fmt:formatNumber
									pattern="#,###" value="${NoSumSales }" /> 원</td>
							<td class="text-right" colspan="4"><fmt:formatNumber
									pattern="#,###" value="${NoSumCommission }" /> 원</td>
						</tr>
						<tr>
						...
						<tr>
							<td class="text-right" colspan="4"><fmt:formatNumber
									pattern="#,###" value="${YesSumSales }" /> 원</td>
							<td class="text-right" colspan="4"><fmt:formatNumber
									pattern="#,###" value="${YesSumCommission }" /> 원</td>
						</tr>
					</c:if>
				</tbody>
			</table>
